...
export default function CollegeComparison() {
  ...
  const colorOf = (college) => colorAt(selected.findIndex((c) => c.id === college.id));

  /* ── Chart data ──────────────────────────────────────────────────── */

  const feesData = useMemo(
    () => selected.map((c, i) => ({ name: c.name, fees: c.fees, fill: colorAt(i) })),
    [selected],
  );

  const placementData = useMemo(
    () => selected.map((c, i) => ({ name: c.name, rate: c.placementRate, fill: colorAt(i) })),
    [selected],
  );
  ...
  const salaryTrendData = useMemo(
    () => YEARS.map((year, yi) => ({
      year,
      ...selected.reduce((acc, c) => ({ ...acc, [c.name]: c.salaryTrend[yi] }), {}),
    })),
    [selected],
  );
  ...
  const bests = useMemo(() => [
    {
      ...
      college: selected.reduce((b, c) => (c.itPrograms > b.itPrograms ? c : b), selected[0]),
      fmt: (c) => `${c.itPrograms} programs`,
    },
    {
      label: 'Lowest Fees',     icon: '💸',
      college: selected.reduce((b, c) => (c.fees < b.fees ? c : b), selected[0]),
      fmt: (c) => fmtMoney(c.fees),
    },
    {
      label: 'Placement Rate',  icon: '🎓',
      college: selected.reduce((b, c) => (c.placementRate > b.placementRate ? c : b), selected[0]),
      fmt: (c) => fmtPct(c.placementRate),
    },
    {
      label: 'Avg Salary',      icon: '💰',
      college: selected.reduce((b, c) => (c.avgSalary > b.avgSalary ? c : b), selected[0]),
      fmt: (c) => fmtMoney(c.avgSalary),
    },
    {
      label: 'Student Rating',  icon: '⭐',
      college: selected.reduce((b, c) => (c.studentRating > b.studentRating ? c : b), selected[0]),
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-x-auto">
          <table className="w-full text-sm min-w-[480px]">
            ...
            <tbody className="divide-y divide-gray-800/60">
              {[
                { emoji: '�', label: 'IT Programs',     key: 'itPrograms',    fmt: (v) => `${v} programs`, lowerBetter: false },
